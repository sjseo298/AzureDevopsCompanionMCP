...
usage() {
  cat <<'USAGE'
Uso: comments_versions_list.sh --project <proj> --work-item-id <id> --comment-id <id> [--help]
...
Ejemplos:
  scripts/curl/wit/comments_versions_list.sh --project "Mi Proyecto" --work-item-id 123 --comment-id 456
USAGE
}
...
URL="${DEVOPS_BASE}/${ENC_PROJECT}/_apis/wit/workItems/${ENC_WI}/comments/${ENC_COMMENT}/versions?api-version=${API_VER_OVERRIDE}"

HDR_FILE=$(mktemp)
BODY_FILE=$(mktemp)
HTTP_CODE=$(curl -sS -u ":${AZURE_DEVOPS_PAT}" -H "Accept: application/json" -D "$HDR_FILE" -o "$BODY_FILE" -w "%{http_code}" "$URL")
