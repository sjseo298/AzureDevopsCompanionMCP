#!/usr/bin/env bash
set -euo pipefail
# Script derivado de api_doc/wit_sections/recycle_bin.md (Get Deleted Work Item)
...
usage(){ cat <<'USAGE'
Uso: recycle_bin_get.sh --project <proyecto> --id <id>

Fuente: api_doc/wit_sections/recycle_bin.md (Get Deleted Work Item)

Descripción:
  Obtiene un work item eliminado de la papelera.
...
Ejemplos:
  scripts/curl/wit/recycle_bin_get.sh --project "Mi Proyecto" --id 12345
USAGE
}
