...
usage() {
  cat <<'USAGE'
Uso: comments_reactions_delete.sh --project <proj> --work-item-id <id> --comment-id <id> --type <like|dislike|heart|hooray|smile|confused> [--help]

Fuente: api_doc/wit_sections/comments_reactions.md

Descripción:
  Elimina una reacción del usuario actual en un comentario.
...
Ejemplos:
  scripts/curl/wit/comments_reactions_delete.sh --project "Mi Proyecto" --work-item-id 123 --comment-id 456 --type like
USAGE
}
