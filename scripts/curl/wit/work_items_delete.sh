...
# Script: Delete single work item (Recycle Bin por defecto, usar --destroy para eliminación permanente)
...
usage() {
  cat <<'USAGE'
Uso: work_items_delete.sh --project <project> --id <id> [--destroy] [--api-version <ver>] [--raw]
USAGE
}

PROJECT=""; ID=""; DESTROY=0; API_VER="7.2-preview"; RAW=0
while [[ $# -gt 0 ]]; do
  case "$1" in
...
[[ -z "$PROJECT" || -z "$ID" ]] && echo "Faltan obligatorios" >&2 && usage && exit 2
